#!/bin/bash
# =============================================================================
# Nginx: Uninstall — purge Nginx + PHP + DB (auto-detects which DB)
#
...
if [ "${KEEP_PACKAGES:-no}" != "yes" ]; then
    ...
    apt-get purge -y 'nginx*' 'php*' >/dev/null 2>&1 || true
    case "$DATABASE" in
        mariadb)
    ...
    if [ "${DELETE_CERTS:-no}" = "yes" ]; then
        apt-get purge -y certbot python3-certbot-nginx >/dev/null 2>&1 || true
    fi
